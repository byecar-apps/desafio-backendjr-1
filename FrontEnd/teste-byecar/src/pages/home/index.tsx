...
export const Home = () => {
  ...
  const [peoplesData, setPeoplesData] = useState<any>(null);
  ...
  useEffect(() => {
    const allPeople = async () => {
      ...
      const list = Object.values(peoples.peoples);
      setListPeoples(list);
    };
    allPeople();
  }, [counter, page]);
          ...
          >
            <Flex direction={"column"} gap={"2rem"}>
              ...
              <Flex alignItems={"center"} justifyContent={"center"} gap={3}>
                {peoplesData && peoplesData.previusPage && (
                  <Button
                    onClick={() => setPage(peoplesData.previusPage)}
                    colorScheme="purple"
                  >{`< ${peoplesData.previusPage}`}</Button>
                )}
                {peoplesData && peoplesData.total !== 0 && <Text>{page}</Text>}
                {peoplesData && peoplesData.nextPage && (
                  <Button
                    onClick={() => setPage(peoplesData.nextPage)}
                    colorScheme="purple"
                  >
                    {`${peoplesData.nextPage} >`}
                  </Button>
                )}
              </Flex>
            </Flex>
